...
//            $firstChannelSession = $firstChannel->channel_session->fetch();
    ...
    public static function reschedule(RestRequest $request): array
    {
        ...
        if ($type === 'only_this') {
            $oldSchedules = Schedule::where( 'id', $id )->fetchAll();
        }else {
            $group_id = Schedule::where( 'id', $id )->select(['group_id'])->fetch();

            $oldSchedules = Schedule::where( 'group_id', $group_id->group_id )->fetchAll();
        }
