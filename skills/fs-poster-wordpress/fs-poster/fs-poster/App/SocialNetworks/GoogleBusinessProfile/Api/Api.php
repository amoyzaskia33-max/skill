...
class Api
{
    ...
    public function sendPost (PostingData $postingData ) : string
	{
        ...
        $posted = Helper::retry(function() use($post, $postingData) {
    ...
    public function apiRequest ( $HTTPMethod, $url, $data = [], $body = '' )
    {
        ...
        if ( isset( $response['error'] ) )
        {
            ...
            if ( isset( $response['error']['details'][0]['errorDetails'][0]['message'] ) )
                $error_msg .= sprintf( ' (%s) ', $response['error']['details'][0]['errorDetails'][0]['message'] );
